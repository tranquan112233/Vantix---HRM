...
export const employeeApi = {
  ...
  uploadPhoto: (id, file) => {
    ...
    return http.post(`/employees/${id}/photo`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
      timeout: 60000,
    })
  },
  downloadPhoto: (id) => http.get(`/employees/${id}/photo`, { responseType: 'blob' }),
  deletePhoto: (id) => http.delete(`/employees/${id}/photo`),
  uploadDocuments: (id, files) => {
    const formData = new FormData()
    files.forEach(file => formData.append('files', file))
    return http.post(`/employees/${id}/documents`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
      timeout: 60000,
    })
  },
  deleteDocument: (employeeId, documentId) => http.delete(`/employees/${employeeId}/documents/${documentId}`),
  downloadDocument: (documentId) => http.get(`/employees/documents/${documentId}/download`, {
    responseType: 'blob',
  }),
}
...
export const contractApi = {
  ...
  uploadSignedFile: (id, file) => {
    const formData = new FormData()
    formData.append('file', file)
    return http.post(`/contracts/${id}/signed-file`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
      timeout: 60000,
    })
  },
  downloadSignedFile: (id) => http.get(`/contracts/${id}/signed-file`, {
    responseType: 'blob',
  }),
  deleteSignedFile: (id) => http.delete(`/contracts/${id}/signed-file`),
}
...
export const payrollApi = {
  ...
  byEmployee: (employeeId) => http.get(`/payrolls/employee/${employeeId}`),
