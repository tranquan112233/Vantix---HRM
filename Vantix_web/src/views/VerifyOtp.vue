...
onMounted(() => {
  ...
  devOtp.value = sessionStorage.getItem('vx_reset_dev_otp') || ''
...
<template>
  <div class="auth-card">
    ...
    <el-alert v-if="devOtp" class="otp-demo" type="info" :closable="false">
      {{ settings.t('auth.demoOtp') }}: <strong>{{ devOtp }}</strong>
    </el-alert>
