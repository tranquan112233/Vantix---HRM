...
public class AttendanceService {
    ...
    public Employee isEmployeeValid(Long id) {
        return employeeRepository.findById(id).orElseThrow(() -> new RuntimeException("Không tìm thấy nhân viên (" + id + ") trên hệ thống."));
    }
    ...
    public Shift getScheduledShiftForToday(Employee employee) {
        ...
        DailyWorkSchedules schedule = dailyWorkSchedulesRepository.findByEmployeeIdAndWorkDate(employee.getId(), today).orElseThrow(() -> new RuntimeException(msgSchedule));
    ...
    public Attendance createAttendanceRecord(Employee employee, Shift shift) {
        ...
        boolean alreadyChecked = attendanceRepository.isAlreadyCheckedIn(employee.getId(), shift.getShiftId(), today);
        if (alreadyChecked) {
    ...
    public Attendance findAttendanceToUpdate(Employee employee) {
        ...
        Attendance att = attendanceRepository.findExistingAttendance(employee.getId(), shift.getShiftId(), today).orElseThrow(() -> new RuntimeException("Bạn chưa Check-in cho ca " + shift.getShiftName() + " ngày " + today + ", không thể Check-out."));
    ...
    public Attendance findPendingAutoCheckOut(Employee employee) {
        ...
        return attendanceRepository.findPendingAttendance(employee.getId(), today).orElseThrow(() -> new RuntimeException("Bạn không có yêu cầu xác nhận công nào đang chờ xử lý trong hôm nay."));
    }
