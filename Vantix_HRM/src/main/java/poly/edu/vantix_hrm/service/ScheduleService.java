...
public class ScheduleService {
    ...
    public List<EmployeeScheduleDTO> getSchedulesForUser(Integer viewerId, int month, int year) {
        ...
        boolean isManager = viewer.getPosition().getPositionId() == 1;

        if (isManager) {
            List<Employee> staffList = employeeRepository.findByDepartment_DepartmentId(viewer.getDepartment().getDepartmentId());
    ...
    private EmployeeScheduleDTO buildEmployeeScheduleDTO(Employee emp, int month, int year, boolean isManager) {
        ...
        MonthlySchedules monthlyEntity = monthlySchedulesRepository.findByEmployee_EmployeeIdAndMonthAndYear(emp.getEmployeeId(), month, year);
    ...
    public void saveDailySchedules(Integer employeeId, int month, int year, List<DailyScheduleDTO> incomingDays) {
