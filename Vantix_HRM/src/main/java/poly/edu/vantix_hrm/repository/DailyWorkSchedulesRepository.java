...
public interface DailyWorkSchedulesRepository extends JpaRepository<DailyWorkSchedules, Integer> {
    ...
    // Dùng cho Attendance
    @Query("SELECT dws FROM DailyWorkSchedules dws " + "WHERE dws.monthlySchedule.employee.employeeId = :employeeId " + "AND dws.workDate = :workDate")
    Optional<DailyWorkSchedules> findByEmployeeIdAndWorkDate(@Param("employeeId") Integer employeeId, @Param("workDate") LocalDate workDate);
}
