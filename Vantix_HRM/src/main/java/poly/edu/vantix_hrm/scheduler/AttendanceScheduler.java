    ...
    public void autoCheckOutScanner() {
        ...
        for (Attendance att : listNeedAutoCheckOut) {
            try {
                ...
                System.out.println("✅ Auto-checkout thành công cho NV: " + att.getEmployee().getId());
            } catch (Exception e) {
                System.err.println("❌ Lỗi xử lý NV " + att.getEmployee().getId() + ": " + e.getMessage());
            }
        }
    }
    ...
    public void autoCheckOutRejected() {
        ...
        for (Attendance att : expiredList) {
            try {
                ...
                System.out.println("❌ Auto-Reject NV: " + att.getEmployee().getId());
            } catch (Exception e) {
                System.err.println("Lỗi xử lý NV " + att.getEmployee().getId() + ": " + e.getMessage());
            }
        }
