        ...
        <div class="dropdown-menu dropdown-menu-end notification-panel shadow p-0">
          <div class="notif-header p-2 border-bottom d-flex justify-content-between align-items-center bg-light">
            ...
            <div class="d-flex gap-2">
              <button v-if="unreadCount > 0" class="btn-action-text text-primary" @click="markAllAsRead">Đọc hết</button>
              <button v-if="notifications.length > 0" class="btn-action-text text-danger" @click="clearAllExceptStarred">Dọn dẹp</button>
            </div>
          </div>
          ...
          <div class="notif-list" style="max-height: 400px; overflow-y: auto;">
            ...
            <div v-for="note in notifications" :key="note.id"
              ...
              <div class="d-flex align-items-start gap-2 flex-grow-1" @click="handleNotifClick(note)" style="cursor: pointer;">
                ...
                <div class="flex-grow-1">
                  ...
                  <div class="notif-msg text-muted" style="font-size: 11.5px; line-height: 1.2;">{{ note.message }}</div>
              ...
              <div class="notif-actions d-flex flex-column gap-1">
                <button class="action-btn" @click.stop="toggleStar(note)" :title="note.starred ? 'Bỏ lưu' : 'Lưu thông báo'">
          ...
          <RouterLink to="/my-notifications" class="notif-footer p-2 d-block text-center border-top text-decoration-none">
...
onUnmounted(() => { notifService.disconnect() })
...
const getNotifIcon = (type) => {
  switch(type) {
    case 'SUMMON': return 'bi-exclamation-triangle-fill'
    case 'TASK': return 'bi-clipboard-check'
    case 'LEAVE': return 'bi-calendar-event'
...
<style scoped>
.notification-panel { width: 340px; border-radius: 12px !important; overflow: hidden; border: 1px solid var(--border); }
.notif-header { background: #f8f9fa; }
.btn-action-text { background: none; border: none; font-size: 11px; font-weight: 600; padding: 0; cursor: pointer; }
.btn-action-text:hover { text-decoration: underline; }

.notif-item { position: relative; transition: all 0.2s ease; }
.notif-item:hover { background: #f8f9fa; }
.notif-item.unread { background: #f0f7ff; border-left: 3px solid #0d6efd; }

.notif-actions { opacity: 0; transition: opacity 0.2s ease; padding-right: 4px; }
.notif-item:hover .notif-actions { opacity: 1; }

.action-btn { background: transparent; border: none; padding: 4px; font-size: 14px; color: #adb5bd; cursor: pointer; border-radius: 4px; }
.action-btn:hover { background: rgba(0,0,0,0.05); color: #212529; }
.bi-star-fill { color: #ffc107 !important; }

.notif-icon-circle { width: 34px; height: 34px; border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.notif-footer { font-size: 12px; font-weight: 600; color: #0d6efd; background: #fff; }
.notif-footer:hover { background: #f8f9fa; }

.app-header { height: 56px; display: flex; align-items: center; justify-content: space-between; padding: 0 16px; background: #fff; border-bottom: 1px solid var(--border); position: sticky; top: 0; z-index: 1000; }
.header-right { display: flex; align-items: center; gap: 8px; }
.icon-btn { width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; border: none; background: transparent; border-radius: 8px; color: #6c757d; cursor: pointer; transition: all 0.2s; }
.icon-btn:hover { background: #f1f3f5; color: #212529; }

.user-btn { display: flex; align-items: center; gap: 8px; border: none; background: transparent; padding: 4px; cursor: pointer; border-radius: 8px; }
.avatar { width: 32px; height: 32px; border-radius: 50%; background: #0d6efd; color: #fff; font-weight: bold; display: flex; align-items: center; justify-content: center; font-size: 14px; }
.user-info { text-align: left; line-height: 1.2; display: flex; flex-direction: column; max-width: 120px; }
.user-name { font-size: 13px; font-weight: 600; color: #343a40; }
.user-email { font-size: 11px; color: #adb5bd; }

.drop-item { display: flex; align-items: center; gap: 10px; padding: 8px 12px; font-size: 13.5px; color: #343a40; text-decoration: none; border-radius: 6px; border: none; background: none; text-align: left; }
