            ...
            <table class="table table-hover align-middle">
              ...
              <tr v-if="filteredTasks.length === 0">
                <td colspan="5" class="text-center py-5 text-muted fw-bold">
                  Không tìm thấy công việc nào của nhân viên này! 🕵️‍♂️
                </td>
              </tr>
            ...
            <div class="selected-box p-3 rounded-4 border shadow-sm mb-4"
              ...
              <div class="bg-white p-3 rounded-3 shadow-sm border mb-3">
                ...
                <div v-if="selectedTask.fileUrl">
                  <button @click="openEvidence(selectedTask.fileUrl)" class="btn btn-sm btn-primary w-100 fw-bold rounded-pill mb-2">
                    📄 Xem file minh chứng
                  </button>
                </div>
              </div>
...
const handleReopen = async (id) => {
  ...
  try {
    await axios.put(`http://localhost:8080/api/tasks/${id}/reopen`);
...
const handleCancel = async (id) => {
  ...
  try {
    await axios.put(`http://localhost:8080/api/tasks/${id}/cancel`);
